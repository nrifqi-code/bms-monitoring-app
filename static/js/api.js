...
const DUMMY = {
  ...
  monitoringLog: (page = 1, perPage = 15) => ({
    data: Array.from({ length: perPage }, (_, i) => ({
      ...
      soh: 98.0,
      temperature: i === 5 ? 45.0 : parseFloat((30 + Math.random() * 2 - 1).toFixed(1)),
    })),
    total: 150,
    page,
    per_page: perPage,
  ...
  alarmHistory: {
    data: [
      { time: '10:18:32', alarm: 'High Temperature Warning', severity: 'Critical', status: 'Active' },
      { time: '09:52:11', alarm: 'Charging Started', severity: 'Info', status: 'Resolved' },
      { time: '09:45:03', alarm: 'System Check', severity: 'Info', status: 'Resolved' },
    ],
    total: 30,
    page: 1,
    per_page: 15,
...
const api = {
  ...
  async getMonitoringLog(page = 1, perPage = 15, filter = 'today') {
    ...
    return apiFetch(`/history/monitoring?page=${page}&per_page=${perPage}&filter=${filter}`);
  },
  async getAlarmHistory(page = 1, perPage = 15, filter = 'today') {
    if (USE_DUMMY) return DUMMY.alarmHistory;
    return apiFetch(`/history/alarms?page=${page}&per_page=${perPage}&filter=${filter}`);
  },
  async getSettings() {
